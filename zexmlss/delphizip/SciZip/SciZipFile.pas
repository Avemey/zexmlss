...
type
  ...
  TLocalFile = packed record
    ...
    CompressedData: TBytes; //variable size
  end;
  ...
  private
    function GetUncompressedBytes(i: integer): TBytes;
    procedure SetUncompressedBytes(i: integer; const Value: TBytes);
  ...
  public
    ...
    property Bytes[i: integer]: TBytes Read GetUncompressedBytes Write SetUncompressedBytes;
...
function ZipCRC32(const Data: TBytes): longword;
...
begin
    for i := 0 to High(Files) do
      ...
      begin
        ...
        fnlen := Length(fn2);
        SetLength(fn, fnlen);
        CharToOemBuff(PChar(fn2), PAnsiChar(fn), fnlen);
        ...
        ZipFileStream.Write(PAnsiChar(fn)^,
    ...
    for i := 0 to High(CentralDirectory) do
      ...
      ZipFileStream.Write(PAnsiChar(fn)^, length(fn));
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          SetLength(fn2, fnlen);
          OEMToCharBuff(PAnsiChar(filename), PChar(fn2), fnlen);
          filename := AnsiString(fn2);
        end;
      end;
    end;
    signature := 0;
...
begin
  Result := AnsiString(StringOf(GetUncompressedBytes(i)));
end;

function TZipFile.GetUncompressedBytes(i: integer): TBytes;
var
...
begin
  ...
  UncompressedStream := TDataStream.Create;
  try {+}
    UncompressedStream.Write(Aheader[1], Length(Aheader));
    UncompressedStream.Write(Files[i].CompressedData[0], Length(Files[i].CompressedData));
    ...
    try {+}
      ...
      ReadBytes := Decompressor.Read(Result[0], Files[i].CommonFileHeader.UncompressedSize);
...
begin
  SetUncompressedBytes(i, BytesOf(Value));
end;

procedure TZipFile.SetUncompressedBytes(i: integer; const Value: TBytes);
var
...
begin
  ...
  {$IFDEF UNICODE}
    SetLength(Files[i].CompressedData, 0);
    //strip the 2 byte headers and 4 byte footers
    for ii := 2 to compressedStream.Size - 5 do
    begin
      SetLength(Files[i].CompressedData, Length(Files[i].CompressedData) + 1);
      Files[i].CompressedData[High(Files[i].CompressedData)]:=compressedStream.Bytes[ii];
    end;
  {$ELSE}
    //strip the 2 byte headers and 4 byte footers
...
begin
  ...
  SetLength(Files[High(Files)].CompressedData, 0); //start with an empty file
...
begin
  ...
  for i := 0 to length(Data) - 1 do
    Result := (Result shr 8) xor (CRCtable[byte(Result) xor Data[i]]);
