...
type
  TZXLSXFileItem = record
    name: string;     //ïóòü ê ôàéëó
    original: string; //èñõîäíàÿ ñòðîêà
    ftype: integer;   //òèï êîíòåíòà
  end;
  ...
  TZXLSXRelations = record
    id: string;       //rID
    ftype: integer;   //òèï ññûëêè
    target: string;   //ññûëêà íà ôàéëà
    fileid: integer;  //ññûëêà íà çàïèñü
    name: string;     //èìÿ ëèñòà
    state: byte;      //ñîñòîÿíèå
    sheetid: integer; //íîìåð ëèñòà
  end;
...
///Differential Formatting
  ...
  private
    FUseStyle: boolean;             //çàìåíÿòü ëè ñòèëü
    FUseColor: boolean;             //çàìåíÿòü ëè öâåò
    FColor: TColor;                 //öâåò ëèíèé
    FLineStyle: TZBorderType;       //ñòèëü ëèíèé
    FWeight: byte;
  protected
  public
    constructor Create();
  ...
  //Èòåì äëÿ äèôôåðåíöèðîâàííîãî ôîðìàòèðîâàíèÿ
  //TODO:
  //      âîçìîæíî, äëÿ excel xml òîæå ïîíàäîáèòñÿ (ïåðåíåñòè?)
  TZXLSXDiffFormattingItem = class(TPersistent)
  private
    FUseFont: boolean;              //çàìåíÿòü ëè øðèôò
    FUseFontColor: boolean;         //çàìåíÿòü ëè öâåò øðèôòà
    FUseFontStyles: boolean;        //çàìåíÿòü ëè ñòèëü øðèôòà
    FFontColor: TColor;             //öâåò øðèôòà
    FFontStyles: TFontStyles;       //ñòèëü øðèôòà
    FUseBorder: boolean;            //çàìåíÿòü ëè ðàìêó
    FBorders: TZXLSXDiffBorder;     //×òî ìåíÿòü â ðàìêå
    FUseFill: boolean;              //çàìåíÿòü ëè çàëèâêó
    FUseCellPattern: boolean;       //Çàìåíÿòü ëè òèï çàëèâêè
    FCellPattern: TZCellPattern;    //òèï çàëèâêè
    FUseBGColor: boolean;           //çàìåíÿòü ëè öâåò çàëèâêè
    FBGColor: TColor;               //öâåò çàëèâêè
    FUsePatternColor: boolean;      //Çàìåíÿòü ëè öâåò øàáëîíà çàëèâêè
    FPatternColor: TColor;          //Öâåò øàáëîíà çàëèâêè
  protected
  public
    constructor Create();
    destructor Destroy(); override;
  ...
  //Äèôôåðåíöèðîâàííîå ôîðìàòèðîâàíèå
...
//Äîïîëíèòåëüíûå ôóíêöèè, íà ñëó÷àé ÷òåíèÿ îòäåëüíîãî ôàéëà
...
//Äîïîëíèòåëüíûå ôóíêöèè äëÿ ýêñïîðòà îòäåëüíûõ ôàéëîâ
...
type
  ...
  TZEXLSXFontArray = array of TZEXLSXFont; //ìàññèâ øðèôòîâ
...
type
  ...
  public
    ...
    property XMLSS: TZEXMLSS read FXMLSS write FXMLSS;                  //õðàíèëèùå
    property ListName: string read FListName write FListName;           //èìÿ ëèñòà
    property RetCode: integer read FRetCode;                            //êîä îøèáêè
    property FileArray[num: integer]: TZXLSXFileItem read GetFileItem;  //ôàéëû
    property ArchFile[num: integer]: string read GetArchFileItem;
    property ArchFilesCount: integer read FArchFilesCount;              //êîë-âî ôàéëîâ â àðõèâå
    property FilesCount: integer read FFilesCount;                      //êîë-âî ôàéëîâ
    property FileType: integer read FFileType write FFileType;          //òèï ôàéëà
    property FileNumber: integer read FFileNumber write FFileNumber;    //íîìåð
    property RelationsCounts[num: integer]: integer read GetRelationsCounts; //êîë-âî â îòíîøåíèõ
    property RelationsCount: integer read FRelationsCount;                   //êîë-âî îòíîøåíèé
    property RelationsArray[num: integer]: TZXLSXRelationsArray read GetRelationsArray; //îòíîøåíèÿ
    property SheetRelationNumber: integer read FSheetRelationNumber;
    property isNeedReadComments: boolean read FNeedReadComments;        //íóæíî ëè ÷èòàòü ïðèìå÷àíèÿ
  end;
...
//Îáíóëÿåò SheetRelations äëÿ òåêóùåãî ëèñòà (åñëè íåáûëî êîììåíòàðèåâ è/èëè ññûëîê)
...
//Âîçâðàùàò íîìåð ôàéëà ñ ïðèìå÷àíèÿìè äëÿ òåêóùåãî ëèñòà
//Åñëè âîçâðàùàåò îòðèöàòåëüíîå ÷èñëî - ïðèìå÷àíèÿ íå îáíîðóæåíû
  ...
  begin
    try
      ...
      begin
        ...
        //Åñëè â êîíòåíò çàáûëè äîáàâèòü îòíîøåíèÿ, íî îíè åñòü â àðõèâå - äîáàâëÿåì.
        //Â òåîðèè òàêîãî áûòü íå äîëæíî, íî ìàëî ëè
        _AddAdditionalFiles();
      ...
      //SheetRelations äëÿ êîíêðåòíîãî ëèñòà (÷òåíèå ññûëîê è ïðèìå÷àíèé)
...
//Âîçâðàùàåò íîìåð Relations èç rels
//INPUT
//  const name: string - òåêñò îòíîøåíèÿ
//RETURN
//      integer - íîìåð îòíîøåíèÿ. -1 - íå îïðåäåëåíî
...
//Âîçâðàùàåò òåêñò Relations äëÿ rels
//INPUT
//      num: integer - íîìåð îòíîøåíèÿ
//RETURN
//      integer - íîìåð îòíîøåíèÿ. -1 - íå îïðåäåëåíî
...
//BooleanToStr äëÿ XLSX //TODO: ïîòîì çàìåíèòü
...
//×èòàåò òåìó (themeXXX.xml)
//INPUT
//  var Stream: TStream                   - ïîòîê ÷òåíèÿ
//  var ThemaFillsColors: TIntegerDynArray - ìàññèâ ñ öâåòàìè çàëèâêè
//  var ThemaColorCount: integer          - êîë-âî öâåòîâ çàëèâêè
//RETURN
//      boolean - true - âñ¸ ïðî÷èòàëîñü óñïåøíî
function ZEXSLXReadTheme(var Stream: TStream; var ThemaFillsColors: TIntegerDynArray; var ThemaColorCount: integer): boolean;
var
  xml: TZsspXMLReaderH;
  maxCount: integer;
...
//×èòàåò ñïèñîê íóæíûõ ôàéëîâ èç [Content_Types].xml
//INPUT
//  var Stream: TStream             - ïîòîê ÷òåíèÿ
//  var FileArray: TZXLSXFileArray  - ñïèñîê ôàéëîâ
//  var FilesCount: integer         - êîë-âî ôàéëîâ
//RETURN
//      boolean - true - âñ¸ ïðî÷èòàëîñü óñïåøíî
function ZEXSLXReadContentTypes(var Stream: TStream; var FileArray: TZXLSXFileArray; var FilesCount: integer): boolean;
var
...
//×èòàåò ñòðîêè èç sharedStrings.xml
//INPUT
//  var Stream: TStream           - ïîòîê äëÿ ÷òåíèÿ
//  var StrArray: TStringDynArray - âîçâðàùàåìûé ìàññèâ ñî ñòðîêàìè
//  var StrCount: integer         - êîë-âî ýëåìåíòîâ
//RETURN
//      boolean - true - âñ¸ îê
function ZEXSLXReadSharedStrings(var Stream: TStream; out StrArray: TStringDynArray; out StrCount: integer): boolean;
var
...
{$IFDEF ZUSE_CONDITIONAL_FORMATTING}
//Ïîëó÷èòü óñëîâíîå ôîðìàòèðîâàíèå è îïåðàòîð èç xlsx
//INPUT
//  const xlsxCfType: string              - xlsx òèï óñëîâíîãî ôîðìàòèðîâàíèÿ
//  const xlsxCFOperator: string          - xlsx îïåðàòîð
//  out CFCondition: TZCondition          - ðàñïîçíàííîå óñëîâèå
//  out CFOperator: TZConditionalOperator - ðàñïîçíàííûé îïåðàòîð
//RETURN
//      boolean - true - óñëîâíîå ôîðìàòèðîâàíèå è îïåðàòîð óñïåøíî ðàñïîçíàíû
...
var
  ...
  //Ïðîâåðèòü òèï óñëîâíîãî ôîðìàòèðîâàíèÿ
  //  out isNeddCheckOperator: boolean - âîçâðàùàåò, íóæíî ëè ïðîâåðÿòü
  //                                     îïåðàòîð
  //RETURN
  //      boolean - true - âñ¸ îê, ìîæíî ïðîâåðÿòü äàëåå
...
//×èòàåò ñòðàíèöó äîêóìåíòà
//INPUT
//  var XMLSS: TZEXMLSS                 - õðàíèëèùå
//  var Stream: TStream                 - ïîòîê äëÿ ÷òåíèÿ
//  const SheetName: string             - íàçâàíèå ñòðàíèöû
//  var StrArray: TStringDynArray       - ñòðîêè äëÿ ïîäñòàíîâêè
//      StrCount: integer               - êîë-âî ñòðîê ïîäñòàíîâêè
//  var Relations: TZXLSXRelationsArray - îòíîøåíèÿ
//      RelationsCount: integer         - êîë-âî îòíîøåíèé
//      ReadHelper: TZEXLSXReadHelper   -
//RETURN
//      boolean - true - ñòðàíèöà ïðî÷èòàëàñü óñïåøíî
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        s := xml.Attributes.ItemsByName['r']; //íîìåð
        if (s <> '') then
        begin
        ...
        _type := xml.Attributes.ItemsByName['t']; //òèï
        ...
        s := xml.Attributes.ItemsByName['s']; //ñòèëü
        if (s > '') then
        ...
        //  d - date
        //  òèï ìîæåò îòñóòñòâîâàòü. Èíòåðïðåòèðóåì â òàêîì ñëó÷àå êàê ZEGeneral
      ...
      begin
        _currCol := 0;
        s := xml.Attributes.ItemsByName['r']; //èíäåêñ ñòðîêè
        if (s > '') then
          if (TryStrToInt(s, t)) then
          begin
            _currRow := t - 1;
        ...
        s := xml.Attributes.ItemsByName['ht']; //â ïîèíòàõ?
        if (s > '') then
        begin
        ...
        s := xml.Attributes.ItemsByName['s']; //íîìåð ñòèëÿ
        if (s > '') then
          if (TryStrToInt(s, t)) then
          begin
            //íóæíî ïîäñòàâèòü íóæíûé ñòèëü
          end;
  ...
  {$IFDEF ZUSE_CONDITIONAL_FORMATTING}
  //×òåíèå óñëîâíîãî ôîðìàòèðîâàíèÿ
  ...
  var
    ...
    var
      ...
      //Íàéòè ñòèëü
      //  ïîêà áóäåì äåëàòü òàê: ïðåäïîëàãàåì, ÷òî âñå ÿ÷åéêè â òåêóùåé îáëàñòè
      //  óñëîâíîãî ôîðìàòèðîâàíèÿ èìåþò îäèí ñòèëü. Áåð¸ì ñòèëü èç ëåâîé âåðõíåé
      //  ÿ÷åéêè, êëîíèðóåì åãî, ïðèìåíÿåì äèôô. ñòèëü, äîáàâëÿåì â õðàíèëèùå ñòèëåé
      //  ñ ó÷¸òîì ïîâòîðîâ.
      //TODO: ïîòîì íóæíî áóäåò ïåðåäåëàòü
      //INPUT
      //      dfNum: integer - íîìåð äèôôåðåíöèðîâàííîãî ôîðìàòèðîâàíèÿ
      //RETURN
      //      integer - íîìåð ïðèìåíÿåìîãî ñòèëÿ
        ...
        begin
         (*
          ...
          operator	    - îïåðàòîð:
                              ...
                              containsText	      ñîäåðæèò òåêñò
                              notContains	        íå ñîäåðæèò
                              beginsWith	        íà÷èíàåòñÿ ñ
                              endsWith	          îêàí÷èâàåòñÿ íà
          text	        -  ??
          timePeriod	  -  ??
          rank	        -  ??
          stdDev  	    -  ??
  ...
  try
    ...
    begin
      ...
      begin
        ...
        //s := xml.Attributes.ItemsByName['paperHeight']; //åñëè óòàíîâëåíû paperHeight è Width, òî paperSize èãíîðèðóåòñÿ
...
//Ïðî÷èòàòü ñòèëè èç ïîòîêà (styles.xml)
//INPUT
//  var XMLSS: TZEXMLSS                   - õðàíèëèùå
//  var Stream: TStream                   - ïîòîê
//  var ThemaFillsColors: TIntegerDynArray - öâåòà èç òåìû
//  var ThemaColorCount: integer          - êîë-âî öâåòîâ çàëèâêè â òåìå
//      ReadHelper: TZEXLSXReadHelper     -
//RETURN
//      boolean - true - ñòèëè ïðî÷èòàëèñü áåç îøèáîê
function ZEXSLXReadStyles(var XMLSS: TZEXMLSS; var Stream: TStream; var ThemaFillsColors: TIntegerDynArray; var ThemaColorCount: integer; ReadHelper: TZEXLSXReadHelper): boolean;
type
  ...
  //   0 - left           ëåâàÿ ãðàíèöà
  //   1 - Top            âåðõíÿÿ ãðàíèöà
  //   2 - Right          ïðàâàÿ ãðàíèöà
  //   3 - Bottom         íèæíÿÿ ãðàíèöà
  //   4 - DiagonalLeft   äèàãîíÿëü îò âåðõíåãî ëåâîãî óãëà äî íèæíåãî ïðàâîãî
  //   5 - DiagonalRight  äèàãîíÿëü îò íèæíåãî ëåâîãî óãëà äî ïðàâîãî âåðõíåãî
...
var
  ...
  //Ïðèâîäèò ê øðèôòó ïî-óìîë÷àíèþ
  //INPUT
  //  var fnt: TZEXLSXFont - øðèôò
  ...
  //Îáíóëÿåò ãðàíèöû
  //  var border: TZXLSXBorder - ãðàíèöû
  ...
  //Ìåíÿ¸ò ìåñòàìè bgColor è fgColor ïðè íåñïëîøíûõ çàëèâêàõ
  //INPUT
  //  var PattFill: TZXLSXFill - çàëèâêà
  ...
  begin
    //åñëè íå ñïëîøíàÿ çàëèâêà - íóæíî ïîìåíÿòü ìåñòàìè öâåòà (bgColor <-> fgColor)
  ...
  //Îáíóëÿåò ñòèëü
  //INPUT
  //  var style: TZXLSXCellStyle - ñòèëü XLSX
  ...
  //TZEXLSXFont â TFont
  //INPUT
  //  var fnt: TZEXLSXFont  - XLSX øðèôò
  //  var font: TFont       - ñòàíäàðòíûé øðèôò
  ...
  //Ïðî÷èòàòü öâåò
  //INPUT
  //  var retColor: TColor      - âîçâðàùàåìûé öâåò
  //  var retColorType: byte    - òèï öâåòà: 0 - rgb, 1 - indexed, 2 - theme
  //  var retLumfactor: double  - ÿðêîñòü
  ...
  begin
    ...
    //
      ...
      result := ZPSolid; //{tut} ïîòîì ïîäóìàòü íàñ÷¸ò ñòèëåé ãðàíèö
  end; //_GetPatternFillByStr

  //Îïðåäåëèòü ñòèëü íà÷åðòàíèÿ ãðàíèöû
  //INPUT
  //  const st: string            - íàçâàíèå ñòèëÿ
  //  var retWidth: byte          - âîçâðàùàåìàÿ øèðèíà ëèíèè
  //  var retStyle: TZBorderType  - âîçâðàùàåìûé ñòèëü íà÷åðòàíèÿ ëèíèè
  //RETURN
  //      boolean - true - ñòèëü îïðåäåë¸í
  ...
  var
    ...
    _currBorder: integer; //òåêóùèé íàáîð ãðàíèö
    _currBorderItem: integer; //òåêóùàÿ ãðàíèöà (ëåâàÿ/ïðàâàÿ ...)
    _color: TColor;
    _isColor: boolean;
  ...
  //×èòàåò ñòèëè (cellXfs è cellStyleXfs)
  //INPUT
  //  const TagName: string           - èìÿ òýãà
  //  var CSA: TZXLSXCellStylesArray  - ìàññèâ ñî ñòèëÿìè
  //  var StyleCount: integer         - êîë-âî ñòèëåé
  ...
  //Ñàìè ñòèëè ?? (èëè äëÿ ÷åãî îíè âîîáùå?)
  ...
  //Êîíâåðòèðóåò RGB â HSL
  //http://en.wikipedia.org/wiki/HSL_color_space
  //INPUT
  //      r: byte     -
  //      g: byte     -
  //      b: byte     -
  //  out h: double   - Hue - òîí öâåòà
  //  out s: double   - Saturation - íàñûùåííîñòü
  //  out l: double   - Lightness (Intensity) - ñâåòëîòà (ÿðêîñòü)
  ...
  //Êîíâåðòèðóåò HSL â RGB
  //http://en.wikipedia.org/wiki/HSL_color_space
  //INPUT
  //      h: double - Hue - òîí öâåòà
  //      s: double - Saturation - íàñûùåííîñòü
  //      l: double - Lightness (Intensity) - ñâåòëîòà (ÿðêîñòü)
  //  out r: byte   -
  //  out g: byte   -
  //  out b: byte   -
  ...
  //Differential Formatting äëÿ xlsx
  procedure _Readdxfs();
  var
    _df: TZXLSXDiffFormattingItem;
    _dfIndex: integer;
  ...
  //Ïðèìåíèòü ñòèëü
  //INPUT
  //  var XMLSSStyle: TZStyle         - ñòèëü â õðàíèëèùå
  //  var XLSXStyle: TZXLSXCellStyle  - ñòèëü â xlsx
...
begin
  ...
  try
    ...
    begin
      ...
      //TODO: ðàçîáðàòüñÿ, ÷åì îòëè÷àþòñÿ cellStyleXfs è cellXfs. ×òî çà cellStyles?
      ...
      if ((xml.TagName = 'cellXfs') and (xml.TagType = TAG_TYPE_START)) then  //ñàìè ñòèëè?
    ...
    //òóò íåçàáûòü ïðèìåíèòü íîìåðà öâåòîâ, åñëè áûëè ââåäåíû
...
//×èòàåò íàçâàíèÿ ëèñòîâ (workbook.xml)
//INPUT
//  var XMLSS: TZEXMLSS                 - õðàíèëèùå
//  var Stream: TStream                 - ïîòîê
//  var Relations: TZXLSXRelationsArray - ñâÿçè
//  var RelationsCount: integer         - êîë-âî
//RETURN
//      boolean - true - íàçâàíèÿ ïðî÷èòàëèñü áåç îøèáîê
function ZEXSLXReadWorkBook(var XMLSS: TZEXMLSS; var Stream: TStream; var Relations: TZXLSXRelationsArray; var RelationsCount: integer): boolean;
var
  xml: TZsspXMLReaderH;
  s: string;
...
//Óäàëÿåò ïåðâûé ñèìâîë + ìåíÿåò âñå ðàçäåëèòåëè íà íóæíûå
//INPUT
//  var FileArray: TZXLSXFileArray  - ôàéëû
//      FilesCount: integer         - êîë-âî ôàéëîâ
...
//×èòàåò ñâÿçè ñòðàíèö/ñòèëåé  (*.rels: workbook.xml.rels è .rels)
//INPUT
//  var Stream: TStream                 - ïîòîê äëÿ ÷òåíèÿ
//  var Relations: TZXLSXRelationsArray - ìàññèâ ñ îòíîøåíèÿìè
//  var RelationsCount: integer         - êîë-âî
//  var isWorkSheet: boolean            - ïðèçíàê workbook.xml.rels
//      needReplaceDelimiter: boolean   - ïðèçíàê íåîáõîäèìîñòè çàìåíÿòü ðàçäåëèòåëü
//RETURN
//      boolean - true - óñïåøíî ïðî÷èòàíî
function ZE_XSLXReadRelationships(var Stream: TStream; var Relations: TZXLSXRelationsArray; var RelationsCount: integer; var isWorkSheet: boolean; needReplaceDelimiter: boolean): boolean;
var
  xml: TZsspXMLReaderH;
  s: string;
...
//×èòàåò ïðèìå÷àíèÿ (äîáàâëÿåò ïðèìå÷àíèÿ íà ïîñëåäíþþ ñòðàíèöó)
//INPUT
//  var XMLSS: TZEXMLSS - õðàíèëèùå
//  var Stream: TStream - ïîòîê äëÿ ÷òåíèÿ
//RETURN
//      boolean - true - âñ¸ íîðìàëüíî
function ZEXSLXReadComments(var XMLSS: TZEXMLSS; var Stream: TStream): boolean;
var
  xml: TZsspXMLReaderH;
  s: string;
...
var
  ...
  //Ïûòàåòñÿ ïðî÷èòàòü rel äëÿ ëèñòà
  //INPUT
  //  const fname: string - èìÿ ôàéëà ëèñòà
  //RETURN
  //      boolean - true - ïðî÷èòàë óñïåøíî
  ...
  begin
    ...
    begin
      ...
      //Ôàéë ñ ïðèìå÷àíèÿìè òàêè ïðèñóòñòâóåò!
      if (b) then
      try
...
begin
  ...
  try
    try
      ...
      //åñëè ïðî÷èòàíî 0 ëèñòîâ - ïðîáóåì ïðî÷èòàòü âñå (íå óäàëîñü ïðî÷èòàòü workbook/rel)
      if (_no_sheets) then
      for i := 0 to FilesCount - 1 do
...
var
  ...
  begin
    lst.Clear();
    lst.Add(ZH.FileArray[num].name); //ñïèñîê ôàéëîâ
    u_zip.UnZipFiles(lst);
  end;

  //Ïûòàåòñÿ ïðî÷èòàòü rel äëÿ ëèñòà
  //INPUT
  //     fnum: integer - Íîìåð ôàéëà ñ ëèñòîì
  //RETURN
  //      boolean - true - ïðî÷èòàë óñïåøíî
...
begin
  ...
  try
    try
      ...
      lst.Clear();
      lst.Add('[Content_Types].xml'); //ñïèñîê ôàéëîâ
...
//    CommentCount: integer             - êîë-âî ñòðàíèö ñ êîììåíòàðèÿìè
//  const PagesComments: TIntegerDynArray- íîìåðà ñòðàíèö ñ êîììåíòàðèÿìè (íóìåðÿöèÿ ñ íóëÿ)
...
{$IFDEF ZUSE_DRAWINGS}
//Ñîçäà¸ò ðèñóíîê (drawing*.xml)
//INPUT
//  var XMLSS: TZEXMLSS                 - õðàíèëèùå
//    Stream: TStream                   - ïîòîê äëÿ çàïèñè
//    Drawing: TZEDrawing               - íàáîð ðèñóíêîâ
//    TextConverter: TAnsiToCPConverter - êîíâåðòåð èç ëîêàëüíîé êîäèðîâêè â íóæíóþ
//    CodePageName: string              - íàçâàíèå êîäîâîé ñòðàíèöè
//    BOM: ansistring                   - BOM
//RETURN
...
var
  ...
  // Àäðåñ ÿ÷åéêè îïðåäåëÿåòñÿ ïî êîîðäèíàòàì
  ...
  // Åñëè ðàçìåð êàðòèíêè óêàçàí, òî àäðåñ ÿ÷åéêè ñäâèãàåòñÿ íà çàäàííûé ðàçìåð
...
//Ñîçäà¸ò ëèñò äîêóìåíòà (sheet*.xml)
//INPUT
//  var XMLSS: TZEXMLSS                 - õðàíèëèùå
//    Stream: TStream                   - ïîòîê äëÿ çàïèñè
//    SheetNum: integer                 - íîìåð ëèñòà â äîêóìåíòå
//    TextConverter: TAnsiToCPConverter - êîíâåðòåð èç ëîêàëüíîé êîäèðîâêè â íóæíóþ
//    CodePageName: string              - íàçâàíèå êîäîâîé ñòðàíèöè
//    BOM: ansistring                   - BOM
//  var isHaveComments: boolean         - âîçâðàùàåò true, åñëè áûëè êîììåíòàðèè (÷òîáû ñîçäàòü comments*.xml)
//  const WriteHelper: TZEXLSXWriteHelper - additional data
//RETURN
//      integer
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        // åñëè òèï ÿ÷åéêè ZEGeneral, òî àòðèáóò îïóñêàåì
...
var
  _xml: TZsspXMLWriterH;        //ïèñàòåëü
  _FontIndex: TIntegerDynArray;  //ñîîòâåòñòâèÿ øðèôòîâ
  _FillIndex: TIntegerDynArray;  //çàëèâêè
  _BorderIndex: TIntegerDynArray;//ãðàíèöû
  ...
  //ßâëÿþòñÿ ëè øðèôòû îäèíàêîâûìè
  ...
  begin
    ...
    Result := true; // åñëè óæ äî ñþäà äîáðàëèñü
  end; //_isFontsEqual

  //Îáíîâëÿåò èíäåêñû â ìàññèâå
  //INPUT
  //  var arr: TIntegerDynArray  - ìàññèâ
  //      cnt: integer          - íîìåð ïîñëåäíåãî ýëåìåíòà â ìàññèâå (íà÷èíàåò ñ 0)
  //                              (ïðåäïîëàãàåòñÿ, ÷òî âîçíèêíåò ñèòóàöèÿ, êîãäà íóæíî áóäåò èñïîëüçîâàòü òîëüêî ÷àñòü ìàññèâà)
  ...
  //ßâëÿþòñÿ ëè çàëèâêè îäèíàêîâûìè
  ...
  begin
    ...
    //ïî êàêîé-òî íåïîíÿòíîé ïðè÷èíå, åñëè â íà÷àëå íåòó äâóõ ñòèëåé çàëèâîê (none + gray125),
    //â ãð¸áàíîì 2010-îì îôèñå ãëþ÷àò çàëèâêè (òî-ëè ÷òîáû ñëîæíî áûëî ñäåëàòü ýêñïîðò â xlsx, òî-ëè
    //êðèâûå ðóêè ó ìåëêîìÿãêèõ ïðîãðàììåðîâ). LibreOffice îòêðûâàåò íîðìàëüíî.
    _WriteBlankFill('none');
    _WriteBlankFill('gray125');

    //TODO:
    //ÂÍÈÌÀÍÈÅ!!! //{tut}
    //â íåêîòîðûõ ñëó÷àÿõ fgColor - ýòî öâåò çàëèâêè (âðîäå äëÿ solid), à â íåêîòîðûõ - bgColor.
    //Ïîòîì íå çàáûòü ðàçîáðàòüñÿ.
    for i := -1 to _StylesCount - 1 do
    if (_FillIndex[i + 1] = -2) then
    begin
      _xml.Attributes.Clear();
...
//        ZPDiagStripe:         s := 'darkUpDark'; ??
  ...
  //Äîáàâëÿåò <xf> ... </xf>
  //INPUT
  //      NumStyle: integer - íîìåð ñòèëÿ
  //      isxfId: boolean   - íóæíî ëè äîáàâëÿòü àòðèáóò "xfId"
  //      xfId: integer     - çíà÷åíèå "xfId"
  ...
  begin
    ...
    _xml.Attributes.Add('fillId', IntToStr(_FillIndex[NumStyle + 1] + 2), false); //+2 ò.ê. ïåðâûìè âñåãäà èäóò 2 ëåâûõ ñòèëÿ çàëèâêè
...
(*  The standard does not specify a default value for the vertical attribute.
  ...
  begin
    ...
    begin
      //×òî-òî íå ñîâñåì ïîíÿòíî, êàêîé èìåííî xfId íóæíî ñòàâèòü. Ïîêà áóäåò 0 äëÿ âñåõ.
...
//Äîáàâèòü Relationship äëÿ rels
//INPUT
//      xml: TZsspXMLWriterH  - ïèñàëêà
...
//Ñîçäà¸ò xl/_rels/workbook.xml.rels   
//INPUT
//    PageCount: integer                - êîë-âî ñòðàíèö
//    Stream: TStream                   - ïîòîê äëÿ çàïèñè
//    TextConverter: TAnsiToCPConverter - êîíâåðòåð èç ëîêàëüíîé êîäèðîâêè â íóæíóþ
...
//Ñîçäà¸ò sharedStrings.xml (ïîêà íå ðåàëèçîâàíî)
//INPUT
//  var XMLSS: TZEXMLSS                 - õðàíèëèùå
//    Stream: TStream                   - ïîòîê äëÿ çàïèñè
//    TextConverter: TAnsiToCPConverter - êîíâåðòåð èç ëîêàëüíîé êîäèðîâêè â íóæíóþ
...
//Ñîõðàíÿåò íåçàïàêîâàííûé äîêóìåíò â ôîðìàòå Office Open XML (OOXML)
//INPUT
//  var XMLSS: TZEXMLSS                   - õðàíèëèùå
...
//Ñîõðàíÿåò äîêóìåíò â ôîðìàòå Open Office XML (xlsx)
//INPUT
//  var XMLSS: TZEXMLSS                   - õðàíèëèùå
